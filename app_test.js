...
class LexFlowApp {
    ...
    async init() {
        try {
            ...
            // Initialize database integration (não bloqueia se falhar)
            try {
                await this.dbIntegration.init();
                console.log('Database integration ativada');
            } catch (dbError) {
                console.log('Database integration em modo fallback:', dbError.message);
            }
            ...
            await this.checkChromeAIStatus();
    ...
    async checkChromeAIStatus() {
        try {
            ...
            if (aiStatus.functional) {
                console.log('✅ Chrome AI está disponível e funcional');
                this.toastSystem.show('Chrome AI detectado e funcionando!', 'success', 3000);
            } else {
                console.log('⚠️ Chrome AI não está disponível - usando modo demonstração');
                this.toastSystem.show('Chrome AI não disponível - modo demonstração ativo', 'warning', 5000);
            }
        } catch (error) {
    ...
    async executeAI() {
        ...
        try {
            ...
            const aiStatus = await this.chromeAI.checkAvailability();
            if (!aiStatus.functional) {
                throw new Error('Chrome AI não está disponível. Verifique as configurações do Chrome Canary e as flags experimentais.');
            }
            ...
            console.log('🤖 Usando AI Provider:', aiProvider, 'com opções:', aiOptions);
            ...
            if (aiProvider === 'summarizer') {
                ...
                Faça um resumo executivo dos artigos fornecidos, destacando os principais direitos e obrigações, 
                com linguagem acessível para apresentação ao cliente.`;
            ...
            // Show fallback result only when AI is truly unavailable
    ...
    generateMockResult() {
        return `
            <div style="background: #fff3cd; border: 1px solid #ffeaa7; border-radius: 8px; padding: 16px; margin-bottom: 20px;">
                <h4 style="color: #856404; margin: 0 0 8px 0;">⚠️ Resultado de Demonstração</h4>
                <p style="color: #856404; margin: 0; font-size: 14px;">Chrome AI não está disponível. Este é um exemplo de como seria a análise com IA real.</p>
            </div>
            ...
            <div style="background: #e3f2fd; border: 1px solid #bbdefb; border-radius: 8px; padding: 16px; margin-top: 20px;">
                <h5 style="color: #1565c0; margin: 0 0 8px 0;">🚀 Para usar IA real:</h5>
                <ol style="color: #1565c0; margin: 0; padding-left: 20px;">
                    <li>Instale o Chrome Canary</li>
                    <li>Habilite as flags: <code>chrome://flags/#prompt-api-for-gemini-nano</code></li>
    ...
    formatAIResult(result) {
        ...
        let formatted = result
            ...
            .replace(/^(.+)$/, '<p>$1</p>'); // Wrap in paragraph if not already wrapped
    ...
    async saveAnalysisToHistory(prompt, result) {
        ...
        // Salva no DataManager (compatibilidade)
        this.historyItems.unshift(historyItem);
        await this.dataManager.saveHistory(this.historyItems);
        
        // Salva no IndexedDB (novo sistema)
        try {
            await this.dbIntegration.saveAnalysis({
                id: historyItem.id,
                timestamp: historyItem.timestamp,
                prompt: prompt,
                result: result,
                articles: this.selectedArticles.map(a => a.number),
                engine: 'summarizer',
                preset: 'resumo-executivo'
            });
        } catch (dbError) {
            console.log('Erro ao salvar no IndexedDB (usando fallback):', dbError.message);
        }
    ...
    updateStats() {
        ...
        // Update database status (se existir elemento)
        this.updateDatabaseStatus();
    }

    /**
     * Atualiza status do banco de dados na interface
     */
    async updateDatabaseStatus() {
        try {
            const status = this.dbIntegration.getStatus();
            const stats = await this.dbIntegration.getStats();
            
            // Adiciona indicador visual do status do banco
            ...
            if (status.active) {
                statusElement.textContent = `DB: ${stats.total || 0} registros`;
                statusElement.style.background = '#2ecc71';
            } else {
                statusElement.textContent = 'DB: Fallback';
                statusElement.style.background = '#e74c3c';
            }
            
        } catch (error) {
            console.log('Erro ao atualizar status do banco:', error);
        }
    }
